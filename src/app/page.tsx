...
const BENEFITS = [
  ...
  {
    icon: Search,
    title: 'Every Industrial Court award, classified',
    body: 'Search the full dismissal corpus — each award indexed against the Termination Index and compiled into seven research headings.',
  },
  {
    icon: FileText,
...
export default function LandingPage() {
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-card/80 backdrop-blur">
        <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <nav className="hidden items-center gap-7 text-sm font-medium text-muted-foreground md:flex">
            ...
            <a href="#corpus" className="transition-colors hover:text-foreground">
              The Data Bank
            </a>
            <Link href="/awards" className="transition-colors hover:text-foreground">
              Search Awards
            </Link>
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="mx-auto grid w-full max-w-6xl items-center gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:py-24">
          <div className="animate-fade-in">
            ...
            <h1 className="display mt-5 text-4xl font-bold leading-[1.08] tracking-tight text-brand-deep sm:text-5xl">
              Every dismissal award.
              <br />
              Seven headings. One search.
            </h1>
            <p className="mt-5 max-w-md text-lg text-muted-foreground">
              IR Assist runs your disciplinary caseload end to end — and grounds every
              decision in the full published corpus of Malaysian Industrial Court awards.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Button asChild size="lg">
                <Link href="/awards">
                  Search the awards <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
          ...
          {/* Hero preview card — the real product surface */}
          <div className="relative animate-fade-in">
            <div className="rounded-2xl border border-border bg-card p-5 shadow-xl">
              <div className="flex items-center justify-between border-b border-border pb-3">
                <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                  <Search className="h-4 w-4 text-primary" /> Termination Index
                </div>
                <span className="nums rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
                  live corpus
                </span>
              </div>
              <div className="space-y-2 py-4">
                {[
                  { label: 'Misconduct', sub: 'Theft, Fraud, Dishonesty · Insubordination · …' },
                  { label: 'Constructive Dismissal', sub: 's.20 (1A) referrals, contract test' },
                  { label: 'Redundancy/Retrenchment', sub: 'reorganisation, closure, VSS' },
                  { label: 'Poor Performance', sub: 'KPIs, capacity, improvement plans' },
                ].map((row) => (
                  <div
                    key={row.label}
              ...
              <div className="flex items-center gap-2 rounded-lg bg-primary/5 px-3 py-2.5 text-sm">
                ...
                <span className="text-muted-foreground">
                  Every award compiled into:{' '}
                  <span className="font-medium text-foreground">
                    Type · Case No · Background · Claimant · Company · Findings · Summary
                  </span>
                </span>
              </div>
            </div>
      ...
      {/* The data bank — the moat, stated plainly */}
      <section id="corpus" className="border-b border-border bg-card">
        <div className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6 md:py-20">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="display text-3xl font-bold tracking-tight text-brand-deep">
              The data bank is the product
            </h2>
            <p className="mt-3 text-muted-foreground">
              Not a cherry-picked sample. The complete published record of Industrial Court
              dismissal awards — mined from the official registry, classified to one taxonomy,
              verbatim from the court&apos;s own words.
            </p>
          </div>
          <div className="mx-auto mt-10 grid max-w-3xl grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              { stat: '8,900+', label: 'dismissal awards, 2002–2026' },
              { stat: '7', label: 'headings per award, in fixed order' },
              { stat: '8+5', label: 'Termination Index & misconduct classes' },
            ].map((s) => (
              <div key={s.label} className="rounded-xl border border-border bg-background p-6 text-center">
                <p className="nums text-3xl font-bold text-primary">{s.stat}</p>
          ...
          <div className="mt-8 text-center">
            <Button asChild variant="outline">
              <Link href="/awards">
                Browse the data bank <ArrowRight className="h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>
      ...
      <section id="audience" className="border-y border-border bg-brand-deep">
        <div className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6">
          <div className="flex flex-col items-center gap-2 text-center">
            <h2 className="display text-2xl font-bold text-white">Who we help</h2>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-20 sm:px-6">
        <div className="flex flex-col items-center gap-5 rounded-2xl border border-border bg-card p-10 text-center shadow-sm">
          ...
          <h2 className="display max-w-xl text-3xl font-bold tracking-tight text-brand-deep">
